...
\begin{abstract}
In many complex systems, constituent agents engage in adversarial behavior without explicit awareness or intent. These "Unaware Adversaries"  ("UA") operate based on localized objectives and feedback, yet their interactions within a shared environment result in mutual disruption, resource competition, or functional conflict. This paper formally defines this phenomenon and situates it within existing literature on systems theory. We present and analyze several case studies—including closed-loop, open-loop, and designed conflicts—to build a taxonomy of these these adversarial typologies. By demonstrating how this framework can be used as a generative tool for analysis and design in addition to diagnostic mitigation, we derive illustrative strategies for problems in fields as diverse as generative modeling and internet routing. The result is a framework intended to help identify, classify, and resolve a pervasive yet under-examined class of system failures.
\end{abstract}

\vspace{1ex}
\noindent\textbf{Keywords:} unaware adversaries, emergent conflict, system dynamics, feedback control loops, multi-agent systems, system failures, system design


\section{Introduction}
Consider a common real-world scenario: an office room equipped with two independent climate control systems. A radiator, governed by a building-wide thermostat, provides heat, while a window-mounted air conditioning unit, with its own separate controls, provides cooling. Each system operates according to its own local feedback loop. If an occupant turns on the A/C to cool a stuffy room while the building's heating system is simultaneously trying to maintain a minimum winter temperature, the two agents enter a state of persistent, mutually negating work—a thermodynamic conflict that neither is designed to recognize. This scenario serves as an intuitive archetype for a class of interactions we term \textbf{"Unaware Adversaries" ("UA").}

In this paper, we formalize a study of systems wherein agents function as antagonists without semantic awareness of their opposition. While the term \textit{"adversary"} traditionally implies intentionality, our focus is on interactions where conflict is an emergent property of environmental coupling and feedback dynamics. While these scenarios resemble the Senge's "Accidental Adversaries" archetype\cite{Senge1990}, this framework focuses on a specific, asemantic subset of conflict, specifically in its treatment of intentionality and awareness. In classical systems dynamics, the adversarial state often arises from a decay in trust or a misinterpretation of a known peer's motives. In contrast, the \textbf{UA} framework addresses systems where the coupling is so indirect or the processing constraints so tight that the "adversary" is effectively invisible.

In many technical systems we find that agents often lack any awareness of a \textit{partner} to begin with. We define Unaware Adversaries not by a breakdown in \textit{cooperation}, but by the existence of an \textbf{Analytic Budget},  which is the reality that individual contributors and automated systems alike have a finite capacity to interpret environmental signals. Here, conflict is not a relational failure, but an architectural one where the \textit{"noise"} of the environment masks the \textit{"signal"} of the opponent. By analyzing these phenomena, we can better understand and mitigate unintended inefficiencies and instabilities in complex, decentralized systems.

This framework is designed for the practitioner - the network engineer, the SRE, the IT architect - who must troubleshoot failures in environments where formal systems thinking is rarely applied and mathematical proofs are all but absent. \textbf{UA} provides a methodological language to describe the \textit{negative space} between components that appear healthy in isolation but \textit{fail in aggregate}.

\section{Background and Related Work}
The concept of \textit{unintended systemic conflict} is not new, and the framework of Unaware Adversaries builds upon several established ideas e.g. from systems theory, control theory, economics, public policy. In economics, the notion of \textbf{negative externality} describes a cost imposed on a third party.  While related, externalities are often unidirectional and do not necessarily involve the reciprocal feedback loop central to our definition of \textbf{UA}. In \textbf{systems theory}, Peter Senge's work on system archetypes is highly relevant\cite{Senge1990}. \textit{Fixes that Fail} and \textit{Shifting the Burden} both describe outcomes of such conflicts. Similarly, the study of \textbf{policy resistance} in public administration explores why policies often fail or backfire due to the complex reactions of the system they are intended to manage. Our framework contributes to this body of work by focusing specifically on the agent-level \textit{mechanism} of the conflict: two or more non-coordinating agents, each pursuing a local objective, coupled through a shared environment. We propose that by classifying the nature of the agents, their coupling, and the resulting impact, we can create a more targeted diagnostic and prescriptive tool than a general archetype provides.

\section{A Framework for Unaware Adversaries}
We formally define \textbf{Unaware Adversaries} as a set of two or more agents or systems that satisfy the following criteria:
...
\begin{description}
    ...
    \item[Bounded Processing Capacity] Each agent has limited attention, computational resources, or analytical capacity to process available environmental signals. Agents may optimize only over a subset of observable state variables due to cognitive or computational constraints. We argue that bounded processing capacity is not incidental; without it, agents \textbf{could}, in principle, infer indirect coupling and cease to be unaware.
    ...
    \item[Indirect Coupling] The actions of one agent affect the state of the shared environment, thereby influencing the sensory inputs and subsequent actions of the other agent(s). There is no explicit communication protocol for de-confliction.
    \item[Antagonistic Effect] The collective behavior of the agents results in a net negative impact on the fulfillment of their respective objectives or the overall system operation (i.e., efficiency, reliability, cost.)
\end{description}
...
Here, $u(t)$ is the control signal (1 if on, 0 if off) for the heater ($H$) and cooler ($C$), $P_H$ and $P_C$ are their respective thermal powers, and $C_H$ and $C_C$ are their electrical power draw (W). The total energy cost, $J$, in the \textit{adversarial configuration is orders of magnitude higher than in a stable configuration, for no improvement in temperature stability}.

\subsection{Email Deliverability Degradation}
\label{sub:email}
...
\subsection{Auto-scaling conflict}
\label{sub:auto-scale}
...
\subsection{Systemic Opposition in Urban 
Planning}
\label{sub:urban}
Perhaps the most societally significant examples of Unaware Adversaries occur in socio-technical systems such as urban planning. Consider two municipal departments with laudable but conflicting mandates:
\begin{itemize}
...
\subsection{Interdomain Routing (BGP) Instability}
\label{sub:bgp}
...
\subsubsection{Specific Case: The YouTube/Pakistan Hijack (2008)}
\label{sssec:bgp-incident}
...
\subsection{The Incentive Principle}
\begin{itemize}
\item Redefine "Winning" to Induce Cooperation
\end{itemize}
...
\textbf{This is the most transformative principle.} Instead of just adding a coordinating layer, \textit{change what it means for an agent to succeed.}


\subsection{The Hierarchy Principle}
\begin{itemize}
    \item Appoint a Single Arbiter for Contested States
\end{itemize}
...
Here we will apply the UA Taxonomy and Principles to the Examples in Section \ref{examples}:

\subsection{Thermostatic Conflict: The Heater-A/C Dyad  (Example  \ref{sub:dyad})}
    The \textbf{UA} Solution: \textbf{The Heater-A/C dyad} is a perfect example of when to apply The Hierarchy Principle. Two simple agents are fighting a direct, symmetric battle over a single, continuous variable: room temperature. Their control logic is too simple to negotiate a truce.
    Institute a higher-level arbiter or a "master controller" that has authority over both agents and manages the contested resource. The solution is a single, smart thermostat that controls both the heat and the A/C. It becomes the sole source of truth for the room's temperature and can make intelligent decisions (e.g., defining a deadband between heating and cooling setpoints) that are impossible for the independent agents. This principle can apply to any system fighting over a fungible resource, from CPU cycles in a computer to water rights in a river basin.

\subsection{Email Deliverability Degradation (Example \ref{sub:email})} 
    The \textbf{UA} Solution: In our \textbf{Email Deliverability} example, the marketing platform's performance metrics (e.g., emails sent) are decoupled from the negative externality it creates (degraded domain reputation). The agent is flying blind to the true system-wide cost of its actions.
    Apply the Observability Principle. Instrument the system to make the shared state visible and, crucially, to integrate that state into each agent's local decision-making context. Don't just provide the marketing team with a dashboard showing their send volume; create a shared, top-level "Domain Health" or "Deliverability Score" that is a KPI for \textit{both} the marketing team and the IT department. When the marketing platform initiates a blast, it should see a real-time dip in a metric it is also responsible for. This makes the invisible visible and forces the unaware adversary to reckon with the consequences of its behavior.

\subsection{Auto-scaling Conflict (Example  \ref{sub:auto-scale})}
    The \textbf{UA} Solution: The \textbf{Auto-scaling Conflict}  exemplifies pure interpretation failure. The feedback loop is closed—both agents receive real-time signals about each other's behavior. But recognizing that "my peer's scaling activity is negating my scaling decision" requires:
\begin{enumerate}
...
By the Observability Principle, our solution here is not merely to surface \textit{more} metrics, but to pre-compute and inject the higher-order signal: a "scaling coordination score" that explicitly flags when peer scaling activities are causally linked. This transforms a complex inference problem into a first-order observable signal.

\subsection{Urban Planning (Example \ref{sub:urban})}
    The \textbf{UA} Solution: In the \textbf{Urban Planning} example, each agent (Department) "winning" by its local definition leads to collective failure (gridlock).
    \textit{"Instead of just adding a coordinating layer, change what it means for an agent to succeed."} In this context, it would mean replacing department-level KPIs like "Level of Service" and "Housing Units Approved" with a single, shared, system-level objective, such as "Minimize average citizen commute time" or "Maximize accessibility score." This forces the departments to find a cooperative, globally optimal solution. 
    This represents not only misaligned incentives, but also a limited optimization scope.    
    
    We can add a formal constraint of a \textbf{Bounded Processing Capacity} to call out the problem:
...
Each agent (Department) requires an  appropriate Attention Budget so that Induced Demand and VKT have bandwidth to process these information signals.

\section{Case Studies (Generative)}
In the next two \textbf{Case Studies}, we intentionally extend the analysis beyond diagnosis and mitigation to sketch speculative constructions. These are not presented as finished algorithms or protocol proposals, nor as claims of empirical superiority. Rather, they serve a narrower purpose: to demonstrate that the \textbf{UA} framework is not merely descriptive, but generative - capable of inspiring non-obvious architectural ideas once a system’s conflict structure is made explicit.
\subsection{Interdomain Routing (BGP) Instability \ref{sub:bgp}}
We show a simple demonstration of how routing stability and  global convergence latency can increase under normal operation.
\subsubsection{In the General Case (\textbf{Ex. \ref{sssec:bpg-general}})}
 The \textbf{UA} Solution: Here, we demonstrate the framework’s usefulness by sketching a hypothetical mitigation strategy for interdomain routing: the \textbf{Gradient-Paced BGP Advertiser (GPBA)}. Rather than impose fixed rate limits, GPBA adapts the pace of route advertisement based on the environmental sensitivity observed at the peer:

\[
\Delta t_{\text{propagation}} \propto \left\| \frac{dR_{\text{remote}}}{dR_{\text{local}}} \right\|
\]

This application of the \textbf{Observability Principle}  discourages volatile updates by throttling agents whose announcements produce outsized ripples in routing state. Unlike traditional filters, GPBA uses a reflexive meta-policy: it applies only when sensitivity is high and relaxes when peers demonstrate robustness or stability. This maintains responsiveness while discouraging feedback-amplifying behavior.

\subsubsection{
Could GPBA Have Mitigated The Youtube/Pakistan Incident of \textbf{Example \ref{sssec:bgp-incident}?}}
...
\begin{description}
    \item[The Problem:] In the standard GAN example, each agent "winning" by its local definition leads to instability (vanishing gradients). 
    \item[The Design Solution:]  Our proposal for the \textbf{Pedagogical GAN} is the canonical example: we shift the Generator's objective from "fool the Discriminator" to "maximize the Discriminator's learning rate." This reframes the adversary not as an opponent to be defeated, but as a student to be taught efficiently.
...
\textit{(Here we will acknowledge - to minimize the risk of causing any confusion - that being adversarial is an in-built aspect of GAN architecture, and that continuing to use the "Unaware Adversary" branding without such acknowledgement may be seen as ironic in this case.)}
...
\begin{itemize}
    ...
    \item \textbf{As the very concept of \textit{"What is an Agent?"} seems to be on the verge of an enormous change with the advent of Agentic AI, the Unaware Adversaries Framework - and many others - will no doubt need to be updated with a special focus on the radical shift in the meaning of \textit{Agent, Intent, Adversarial, etc.} that is developing now}. 
\end{itemize}
